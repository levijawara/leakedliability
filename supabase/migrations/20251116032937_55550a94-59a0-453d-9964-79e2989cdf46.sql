-- Phase 1: Add plateau_days column to producers table
ALTER TABLE producers
ADD COLUMN plateau_days INTEGER DEFAULT 0;

-- Phase 2: Replace calculate_pscs_score function with plateau penalty logic
...
BEGIN
  ...
  -- NEW: Plateau detection and penalty
  SELECT pscs_score, COALESCE(plateau_days, 0) 
  INTO stored_score, current_plateau_days
  FROM producers 
  WHERE id = producer_uuid;
  
  -- If the rounded score matches the stored score, we're on a plateau
  ...
  -- CRITICAL: Update BOTH plateau_days AND pscs_score
  UPDATE producers
  SET 
    plateau_days = current_plateau_days,
    pscs_score = final_score
  WHERE id = producer_uuid;
  
  -- Ensure score stays within bounds (preserving decimal precision)
  RETURN GREATEST(0, LEAST(max_score, ROUND(final_score, 2)));
END;
$function$;