-- Fix: calculate_pscs_score should NOT update producers table directly
-- The trigger will handle the update
...
BEGIN
  ...
  -- Ensure score stays within bounds (preserving decimal precision)
  final_score := GREATEST(0, LEAST(max_score, ROUND(final_score, 2)));
  
  -- Return both score and plateau_days as a composite
  -- We'll update the trigger to handle the plateau_days update
  RETURN final_score;
END;
$function$;

-- Create trigger to update plateau_days when PSCS is recalculated
CREATE OR REPLACE FUNCTION update_plateau_days()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  -- Calculate plateau days based on score change
  ...
  -- Reset plateau days if no unpaid debts
  IF NOT EXISTS (
...
DROP TRIGGER IF EXISTS trigger_update_plateau_days ON producers;
CREATE TRIGGER trigger_update_plateau_days
  BEFORE UPDATE OF pscs_score ON producers
  FOR EACH ROW
  EXECUTE FUNCTION update_plateau_days();