-- Phase 1: Fix Data Integrity - Backfill missing closed_date values
UPDATE payment_reports
SET closed_date = COALESCE(payment_date, updated_at::date)
WHERE status = 'paid' AND closed_date IS NULL;

-- Update last_closed_date on all affected producers
UPDATE producers p
SET last_closed_date = (
...
-- Phase 2: Modify PSCS Calculation Function with Recovery Logic
...
BEGIN
  -- NEW: Check if producer has ANY unpaid debts
  SELECT COUNT(*) INTO unpaid_report_count
  FROM payment_reports
  WHERE producer_id = producer_uuid AND status != 'paid';
  
  -- NEW: If producer has zero unpaid debts, apply recovery formula
  IF unpaid_report_count = 0 THEN
    -- Get last_closed_date from producer record
    SELECT 
    ...
    -- If no last_closed_date (never had paid debt), use 0 days clean
    IF days_clean IS NULL THEN
      days_clean := 0;
    END IF;
    
    -- Calculate recovery ratio (capped at 1.0)
    recovery_ratio := LEAST(days_clean::NUMERIC / recovery_days, 1.0);
    
    -- Apply linear recovery formula: PSCS = 700 + (300 * ratio)
    RETURN ROUND(recovery_base + (recovery_max - recovery_base) * recovery_ratio, 0);
  END IF;
  
  -- EXISTING CODE: Load config for penalty calculation
  ...
  -- Calculate CREW History Penalty (LINEAR DROP-OFF: full forgiveness at 30 days)
  SELECT 
    COALESCE(
      SUM(
        CASE
  ...
  -- Calculate VENDOR History Penalty (LINEAR DROP-OFF: full forgiveness at 30 days, heavier)
  SELECT 
    COALESCE(
      SUM(
        CASE
  ...
  -- Calculate CREW Delinquency Ratio Penalty (FIXED: reduced for ≤2 debts)
  ...
  -- Calculate VENDOR Delinquency Ratio Penalty (FIXED: reduced for ≤2 debts)
...
-- Phase 4: Trigger PSCS Recalculation for All Producers
UPDATE producers
